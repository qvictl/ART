...
protected:
  ...
  void populate_tree(const std::string &root_path,
                     const Gtk::TreeModel::Row *parent_row = nullptr) {
    try {
      for (const auto &entry : fs::directory_iterator(root_path)) {
        if (should_display(entry)) {
          // Add a row for the current entry
          Gtk::TreeModel::Row row;
          if (parent_row) {
            row = *(tree_store->append(parent_row->children()));
          } else {
            row = *(tree_store->append());
          }
          row[columns.name] = get_display_name(entry);
          row[columns.path] = entry.path().string();

          // If the entry is a directory, recursively populate its contents
          if (entry.is_directory()) {
            populate_tree(entry.path().string(), &row);
          }
        }
      }
